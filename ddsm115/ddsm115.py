...
class MotorControl:
	...
	def read_reply(self, _id, timeout=0.01):
		...
		if the head bytes are correct then data field will be stored in array. 
		And finally the index 10 is crc will be checked to confirm, the data is completely/correctly sent. 

		timeout is an amount of time to ignore reading, because incoming bytes sometimes are not correct.
		So we repeatedly receive and check index. If finally the timeout is exceeded, then we use the previous value.
		"""
		...
		while not got_reply:
			...
			try:
				...
				print_warning(e)
				res = b''  # Ensure 'res' is defined if an exception occurs

			if len(res) != 0:
				# First byte: check if ring_buffer is empty and the byte matches the ID.
				if (len(ring_buffer) == 0) and (res == _id.to_bytes(1, 'big')):
					ring_buffer.append(int.from_bytes(res, 'big'))

				# Second byte: check if it is the expected mode value (0x02 in this case).
				elif (len(ring_buffer) == 1) and (res == b'\x02'):
					ring_buffer.append(int.from_bytes(res, 'big'))

				# For subsequent bytes, if ring_buffer is not yet full (10 bytes)
				elif (len(ring_buffer) != 10) and (len(ring_buffer) >= 2):
					ring_buffer.append(int.from_bytes(res, 'big'))

					# Once ring_buffer reaches 10 bytes, check CRC.
					if len(ring_buffer) == 10:
						...
						if crc_value == crc_check:
							...
							# Save these values as the previous feedback.
				...
				# If data doesn't match expected format, reset the ring_buffer.
				else:
					ring_buffer = bytearray()
			...
			if period > timeout:
				got_reply = True
				# Use previous feedback values if timeout is exceeded.
				ID = _id
				mode = 0
